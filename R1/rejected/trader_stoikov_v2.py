...
def trade_product(product: str, state: TradingState, trader_data: dict) -> List[Order]:
    ...
    # Reservation price — FIX v2 : drift per-tick (pas cumulé)
    drift = float(params.get("drift", 0.0))
    drift_horizon = float(params.get("drift_horizon", 5.0))  # 5 ticks forward view
    reservation = s - q * gamma * sigma ** 2 * time_left + drift * drift_horizon

...
class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        # === PROBE LOGGING (v31 behaviour unchanged, extra logs only) ===
        # Fair value reference per product (Osm=10000 fixed, Pep=kalman stored in trader_data)
        fv_osm = 10000.0
        pep_state = trader_data.get("INTARIAN_PEPPER_ROOT", {})
        fv_pep = None
        if isinstance(pep_state.get("kf"), dict):
            fv_pep = float(pep_state["kf"].get("x", 0))

        ts = state.timestamp
        pos = state.position or {}

        # 1. Log OWN fills (seen via own_trades) with edge vs fv
        for product in ("ASH_COATED_OSMIUM", "INTARIAN_PEPPER_ROOT"):
            fv = fv_osm if product == "ASH_COATED_OSMIUM" else fv_pep
            if fv is None: continue
            for tr in state.own_trades.get(product, []) or []:
                buyer = getattr(tr, "buyer", "") or ""
                seller = getattr(tr, "seller", "") or ""
                side = "B" if buyer == "SUBMISSION" else ("S" if seller == "SUBMISSION" else "?")
                edge = float(tr.price) - fv
                print(f"FIL ts={ts} p={product[:3]} side={side} price={tr.price} edge={edge:+.1f} qty={tr.quantity} bot_cp=\"{seller if side=='B' else buyer}\"")

        # 2. Log ALL market trades with edge vs fv + bot IDs if any
        for product in ("ASH_COATED_OSMIUM", "INTARIAN_PEPPER_ROOT"):
            fv = fv_osm if product == "ASH_COATED_OSMIUM" else fv_pep
            if fv is None: continue
            for tr in state.market_trades.get(product, []) or []:
                ...
                seller = getattr(tr, "seller", "") or ""
                edge = float(tr.price) - fv
                print(f"MKT ts={ts} p={product[:3]} price={tr.price} edge={edge:+.1f} qty={tr.quantity} b=\"{buyer}\" s=\"{seller}\"")

        # 3. Log full book snapshot every 1000 ts (100 samples per day)
        if ts % 1000 == 0:
            for product in ("ASH_COATED_OSMIUM", "INTARIAN_PEPPER_ROOT"):
                depth = state.order_depths.get(product)
                if depth is None: continue
                fv = fv_osm if product == "ASH_COATED_OSMIUM" else fv_pep
                if fv is None: fv = 0.0
                bids = sorted(depth.buy_orders.items(), reverse=True)[:3]
                asks = sorted(depth.sell_orders.items())[:3]
                bids_s = "|".join(f"{p}@{v}" for p, v in bids)
                asks_s = "|".join(f"{p}@{-v}" for p, v in asks)
                print(f"BOOK ts={ts} p={product[:3]} fv={fv:.1f} pos={pos.get(product, 0)} BIDS:{bids_s} ASKS:{asks_s}")

        # 4. Log our orders emitted this tick (pattern: what we POST at each edge)
        if ts % 500 == 0:  # every 500 ts to limit volume
            for product, orders_list in result.items():
                if not orders_list: continue
                fv = fv_osm if product == "ASH_COATED_OSMIUM" else fv_pep
                if fv is None: continue
                for o in orders_list:
                    edge = float(o.price) - fv
                    side = "B" if o.quantity > 0 else "S"
                    print(f"OUR ts={ts} p={product[:3]} side={side} price={o.price} edge={edge:+.1f} qty={abs(o.quantity)}")

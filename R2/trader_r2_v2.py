...
PRODUCT_PARAMS: Dict[str, dict] = {
    ...
    "INTARIAN_PEPPER_ROOT": {
        **DEFAULT_PARAMS,
        ...
        "log_bot_autopsy": True,
        # v33 (R2) : PEPPER CYCLING INTRADAY
        # Idée : quand on est long Pepper (bootstrap +80 ou mid-journée),
        # vendre partiellement sur spikes haussiers locaux, puis racheter sur dips.
        # Le profit vient de la volatilité intraday (std 817) autour du trend.
        # OFF par défaut — activer via grid search validé.
        "enable_cycling": False,
        "cycle_ma_window": 50,            # fenêtre MA pour computer deviation
        "cycle_sell_trigger": 20.0,       # mid > MA + trigger → sell
        "cycle_buy_trigger": -15.0,       # mid < MA + trigger → buy back
        "cycle_sell_qty": 20,             # qty par cycle sell
        "cycle_buy_qty": 20,              # qty par cycle buy
        "cycle_hold_min": 40,             # on garde au moins ce nb long (preserve HODL)
        "cycle_cooldown": 300,            # cooldown ts entre deux cycles (anti-oscillation)
    },
}
...
def _pepper_cycling(product: str, state: TradingState, trader_data: dict, params: dict,
                    position: int, buy_used: int, sell_used: int, limit: int) -> List[Order]:
    """
    v33 (R2) : Pepper cycling intraday.
    Vend partiellement sur spikes haussiers locaux, rachète sur dips.
    Profit expected from intraday volatility (std 817) around the +1000 daily drift.

    Ne trigger QUE si :
    - enable_cycling=True
    - ma_window samples dispo dans mids
    - cooldown expiré depuis dernier cycle
    - position > hold_min (pour sell) ou position < limit (pour buy)
    """
    if not params.get("enable_cycling", False):
        return []

    orders: List[Order] = []
    pstate = trader_data.setdefault(product, {})
    mids = pstate.get("mids", [])

    ma_window = int(params.get("cycle_ma_window", 50))
    if len(mids) < ma_window:
        return []  # warmup, pas assez de data

    ma = sum(mids[-ma_window:]) / ma_window
    current_mid = mids[-1]
    deviation = current_mid - ma

    sell_trigger = float(params.get("cycle_sell_trigger", 20.0))
    buy_trigger = float(params.get("cycle_buy_trigger", -15.0))
    sell_qty = int(params.get("cycle_sell_qty", 20))
    buy_qty = int(params.get("cycle_buy_qty", 20))
    hold_min = int(params.get("cycle_hold_min", 40))
    cooldown = int(params.get("cycle_cooldown", 300))

    last_cycle_ts = int(pstate.get("last_cycle_ts", -10**9))
    if state.timestamp - last_cycle_ts < cooldown:
        return []

    current_pos = position + buy_used - sell_used
    ...
    # SPIKE HAUT : vendre partiellement (on vide au best bid) — dès qu'on a plus que hold_min
    if deviation > sell_trigger and current_pos > hold_min and bb is not None:
        max_sell = min(sell_qty, current_pos - hold_min)
        if max_sell > 0:
            orders.append(Order(product, bb, -max_sell))
            pstate["last_cycle_ts"] = state.timestamp
            pstate["last_cycle_side"] = "SELL"

    # DIP BAS : racheter (on prend au best ask) — jusqu'à limit
    elif deviation < buy_trigger and current_pos < limit and ba is not None:
        max_buy = min(buy_qty, limit - current_pos)
        if max_buy > 0:
            orders.append(Order(product, ba, max_buy))
            pstate["last_cycle_ts"] = state.timestamp
            pstate["last_cycle_side"] = "BUY"
...
def trade_product(product: str, state: TradingState, trader_data: dict) -> List[Order]:
    ...
    # v33 (R2) : PEPPER CYCLING — orders additionnels sur spikes/dips
    # Appel après tout le reste pour voir la position effective post-MAKE/TAKE.
    # _cap_gross_orders au return du Trader.run protégera contre overshoot.
    if product == "INTARIAN_PEPPER_ROOT":
        cycle_orders = _pepper_cycling(product, state, trader_data, params,
                                        position, buy_used, sell_used, limit)
        if cycle_orders:
...
class Trader:
    ...
    def bid(self) -> int:
        # R2 MAF : bid 300 XIREC pour +25% volume si top 50% des bidders.
        # EV live day 0 estimé : +1030 gain brut − 300 bid = +730 net.
        # Break-even à 1030. Bid < médiane probable (~300-500) tout en étant > 0-bidders (~30-40% des teams).
        return 300

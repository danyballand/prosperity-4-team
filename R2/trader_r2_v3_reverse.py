...
PRODUCT_PARAMS: Dict[str, dict] = {
    "ASH_COATED_OSMIUM": {
        **DEFAULT_PARAMS,
        ...
        # v3_reverse (SIGN-FLIP TEST) : id_markout Osm mais direction INVERSÉE
        # Si bot BUY rentable → on SELL (contrarian)
        # Si bot SELL rentable → on BUY
        # But : diagnostique le mystère +823 XIREC de v3a
        # - Si reverse gagne aussi → side effect non-directionnel
        # - Si reverse perd symétriquement → signal directionnel (follow = bonne dir)
        "id_markout": True,
        "id_markout_reverse": True,    # <<< REVERSE FLAG (nouveau)
...
def trade_product(product: str, state: TradingState, trader_data: dict) -> List[Order]:
    ...
    # v3_reverse : si params.id_markout_reverse=True, inverse le signe (contrarian)
    if params.get("id_markout", False):
        ...
        reverse = bool(params.get("id_markout_reverse", False))
        if informed == "LONG":
            if reverse:
                target_bias = max(-limit, min(target_bias, -id_target))  # CONTRARIAN : bot buy → on sell
                print(f"IDMARK_REV {product} ts={state.timestamp} bot=LONG → we SHORT target={-id_target}")
            else:
                target_bias = min(limit, max(target_bias, id_target))
        elif informed == "SHORT":
            if reverse:
                target_bias = min(limit, max(target_bias, id_target))   # CONTRARIAN : bot sell → on buy
                print(f"IDMARK_REV {product} ts={state.timestamp} bot=SHORT → we LONG target={id_target}")
            else:

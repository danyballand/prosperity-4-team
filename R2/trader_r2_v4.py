...
PRODUCT_PARAMS: Dict[str, dict] = {
    ...
    "INTARIAN_PEPPER_ROOT": {
        **DEFAULT_PARAMS,
        ...
        "log_bot_autopsy": True,
        # v4 (R2 CODEX ALPHA) : Pepper snap-back trend-residual
        # Trend ex-ante = anchor + 0.001/tick (vérifié sur 3 jours R2, slope=0.001000 exact)
        # Quand mid s'écarte > threshold du trend → expected snap-back.
        # Entry aggressive : sell @ bid si resid>+thresh, buy @ ask si resid<-thresh.
        # Validé backtest R2 : threshold=5 → +3.71 ticks/trade, n=656, total +2,433 ticks sur 3j.
        "snap_back_enabled": True,
        "snap_back_slope": 0.001,            # ticks/tick (fix sur 3 jours R2)
        "snap_back_threshold": 5.0,          # |resid| > 5 → trigger
        "snap_back_qty": 20,                 # qty par trigger (agressif au bid/ask)
        "snap_back_hold_min": 40,            # position min à conserver (préserve HODL base)
    },
}
...
def trade_product(product: str, state: TradingState, trader_data: dict) -> List[Order]:
    ...
    # v4 (R2 CODEX ALPHA) : PEPPER SNAP-BACK
    # Utilise L1 mid = (bb+ba)/2 (pas wall_mid) car c'est ce que Codex utilise.
    # Anchor (L1 mid au tick 0) + slope=0.001/tick = trend ex-ante.
    if (product == "INTARIAN_PEPPER_ROOT"
        ...
        # L1 mid (pas wall_mid)
        l1_mid = (bb + ba) / 2.0
        # Anchor : on prend session_anchor mais si c'était wall_mid initial, on veut L1 mid.
        # Ici on garde session_anchor pour cohérence avec le code existant (initialisé au tick 0).
        ...
        resid_sb = l1_mid - trend_sb
        thresh_sb = float(params.get("snap_back_threshold", 5.0))
        qty_sb = int(params.get("snap_back_qty", 20))
        hold_min_sb = int(params.get("snap_back_hold_min", 40))
        cur_pos_sb = position + buy_used - sell_used

        if resid_sb >= thresh_sb and cur_pos_sb > hold_min_sb:
            max_sell_sb = min(qty_sb, cur_pos_sb - hold_min_sb)
            if max_sell_sb > 0:
                orders.append(Order(product, bb, -max_sell_sb))
                sell_used += max_sell_sb
                print(f"SNAPSELL ts={state.timestamp} mid={l1_mid:.1f} trend={trend_sb:.1f} resid={resid_sb:+.1f} pos={cur_pos_sb}->{cur_pos_sb - max_sell_sb} qty={max_sell_sb}")
        elif resid_sb <= -thresh_sb and cur_pos_sb < limit:
            max_buy_sb = min(qty_sb, limit - cur_pos_sb)
            if max_buy_sb > 0:
                orders.append(Order(product, ba, max_buy_sb))
                buy_used += max_buy_sb
                print(f"SNAPBUY ts={state.timestamp} mid={l1_mid:.1f} trend={trend_sb:.1f} resid={resid_sb:+.1f} pos={cur_pos_sb}->{cur_pos_sb + max_buy_sb} qty={max_buy_sb}")

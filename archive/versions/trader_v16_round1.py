...
def _detect_informed_live(product: str, state: TradingState, trader_data: dict) -> str:
    """
...
def trade_product(product: str, state: TradingState, trader_data: dict) -> List[Order]:
    ...
    # v16 : INFORMED OVERRIDE
    if params.get("informed_detection", False):
        informed = _detect_informed_live(product, state, trader_data)
        if informed == "LONG":
            target_bias = min(limit, max(target_bias, 60))  # push long
        elif informed == "SHORT":

...
def trade_product(product: str, state: TradingState, trader_data: dict) -> List[Order]:
    ...
    if not near_limit:
        ...
        if params.get("triple_edge", False) and remaining_buy >= 3 and remaining_sell >= 3:
            # v35 : pyramide INVERSÉE (Gemini) — 70% au prix le plus profond
            # (hypothèse : bot adverse a hard limit à 99 ticks, 70% au meilleur prix pour nous)
            pyramid_pct = params.get("pyramid_pct", (55, 30))  # anciens: (55, 30, reste)
            p1, p2 = pyramid_pct
            b1 = remaining_buy * p1 // 100
            b2 = remaining_buy * p2 // 100
            b3 = remaining_buy - b1 - b2
            s1 = remaining_sell * p1 // 100
            s2 = remaining_sell * p2 // 100

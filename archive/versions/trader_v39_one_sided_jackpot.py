...
PRODUCT_PARAMS: Dict[str, dict] = {
    "ASH_COATED_OSMIUM": {
        **DEFAULT_PARAMS,
        ...
        "clearing_threshold": 0.10,       # v38 : clearing plus aggressif (était 0.20)
        "clearing_price_edge": -1,        # v38 Gemini piste #3 : TAKE NÉGATIF — on paye 1 tick pour vider
        "clearing_urgent_fraction": 0.40, # v38 : urgent plus tôt (était 0.60)
        ...
        # v35 Gemini INVERSÉE testée LIVE : -377 XIREC. Hypothèse "limite 99" fausse. Revert.
        "pyramid_pct": (55, 30),   # 55/30/15 (config v31 champion)
        # v39 Codex : ONE-SIDED JACKPOT — ladder profonde ancrée quand book one-sided (7.85% du temps)
        "far_one_sided": True,
        "far_anchor_fixed": True,
        "far_bid_edge": 97,
        "far_ask_edge": 97,
        "far_offsets": (0, 1, 2),
        "far_weights": (70, 20, 10),
    },
    "INTARIAN_PEPPER_ROOT": {
        **DEFAULT_PARAMS,
        "position_limit": 80,
        "take_width": 3,
        "make_edge": 3,     # v38 : retour Pepper edge=3 (v37 a prouvé que edge=50 ne capture rien)
        ...
        "use_bayes_ap": False,
        "use_bayes_ar_overlay": True,  # v33 Codex: BayesAR 15% blend sur Pepper
        "use_obi_gate": True,          # v33 Codex: top-OBI gate sur Pepper
        "bayes_window": 80,
...
def _top_obi(depth: OrderDepth) -> float:
    """v33 Codex: Top-of-book imbalance, corrèle +0.646 avec next mid."""
    bb, ba = _best_bid(depth), _best_ask(depth)
    if bb is None or ba is None:
        return 0.0
...
def _split_ladder_qty(total: int, weights) -> List[int]:
    """Répartit total entre weights, premier level récupère le reste."""
    total = max(0, int(total))
    if total <= 0:
        return []
    ws = [max(0, int(w)) for w in weights]
    s = sum(ws) or 1
    qtys = [total * w // s for w in ws]
    if qtys:
        qtys[0] += total - sum(qtys)
    return qtys


def _emit_ladder(orders: List[Order], product: str, side: str,
                 base_px: int, total_qty: int, offsets, weights) -> None:
    """Émet une ladder d'ordres avec offsets par rapport à base_px."""
    for off, qty in zip(offsets, _split_ladder_qty(total_qty, weights)):
        if qty <= 0:
            continue
        px = int(base_px - off) if side == "BUY" else int(base_px + off)
        if px <= 0:
            continue
        orders.append(Order(product, px, qty if side == "BUY" else -qty))


def _bucket_bias(product: str, ts: int) -> float:
    """v33 Codex: Osmium bias par bucket temporel. Stable 3/3 jours."""
    if product != "ASH_COATED_OSMIUM":
        return 0.0
    b = min(9, max(0, ts // 100000))
    stable = {4: 2.2, 7: -3.5}
    return 0.35 * stable.get(b, 0.0)


def _bayes_ar_short_fair(product, depth, params, trader_data, horizon=20):
    """v33 Codex: BayesAR short-horizon Pepper. RMSE 4.85 vs drift 6.37."""
    z = _microprice(depth) or _wall_mid(depth, params.get("min_wall_volume", 5))
    if z is None:
        return None
    pstate = trader_data.setdefault(product, {})
    obs = pstate.setdefault("bar_obs", [])
    obs.append(float(z))
    if len(obs) > 81:
        del obs[:len(obs)-81]
    if len(obs) < 5:
        return z
    ds = [obs[i] - obs[i-1] for i in range(1, len(obs))]
    mu = (40.0 * 0.10 + sum(ds)) / (40.0 + len(ds))
    res = [d - mu for d in ds]
    num = sum(res[i] * res[i-1] for i in range(1, len(res)))
    den = sum(res[i-1] * res[i-1] for i in range(1, len(res)))
    phi = (-0.5 * 25.0 + num) / (25.0 + den) if den > 1e-9 else -0.5
    phi = max(-0.65, min(0.65, phi))
    ar = phi * (1.0 - phi ** horizon) / (1.0 - phi) * res[-1] if abs(1.0 - phi) > 1e-9 else 0.0
    return float(z) + horizon * mu + ar
...
def trade_product(product: str, state: TradingState, trader_data: dict) -> List[Order]:
    ...
    if params.get("fixed_fv") is not None:
        # v33 Codex : bucket bias Osmium stable (bucket 4 +2.2, bucket 7 -3.5)
        fv = float(params["fixed_fv"]) + (_bucket_bias(product, state.timestamp) if params.get("use_bucket_bias", False) else 0.0)
    ...
    # v33 Codex : BayesAR overlay Pepper (15% blend)
    if product == "INTARIAN_PEPPER_ROOT" and params.get("use_bayes_ar_overlay", False):
        bayes_fv = _bayes_ar_short_fair(product, depth, params, trader_data, horizon=20)
        if bayes_fv is not None:
            fv = 0.85 * fv + 0.15 * bayes_fv

    # v33 Codex : Top-OBI tactical gate
    top_obi = _top_obi(depth)
    ...
    if not near_limit:
        ...
        # v39 Codex : ONE-SIDED JACKPOT — quand le book a un côté vide,
        # on ancre une ladder profonde directement sur FV fixe (10000)
        # Data: books one-sided = 7.85% du temps sur Osmium
        far_bid_mode = bool(params.get("far_one_sided", False) and bb is None)
        far_ask_mode = bool(params.get("far_one_sided", False) and ba is None)
        if far_bid_mode or far_ask_mode:
            if params.get("far_anchor_fixed", False) and params.get("fixed_fv") is not None:
                far_anchor = float(params["fixed_fv"])
            else:
                far_anchor = fv + skew
            if far_bid_mode:
                make_bid_price = int(round(far_anchor - int(params.get("far_bid_edge", make_edge))))
                if ba is not None:
                    make_bid_price = min(make_bid_price, ba - 1)
            if far_ask_mode:
                make_ask_price = int(round(far_anchor + int(params.get("far_ask_edge", make_edge))))
                if bb is not None:
                    make_ask_price = max(make_ask_price, bb + 1)
        ...
        # v33 Codex : Top-OBI tactical gate (réduit un côté selon imbalance forte)
        if params.get("use_obi_gate", False):
            if product == "ASH_COATED_OSMIUM":
                if top_obi > 0.45:
                    remaining_sell = remaining_sell * 70 // 100
                elif top_obi < -0.45:
                    remaining_buy = remaining_buy * 70 // 100
            elif product == "INTARIAN_PEPPER_ROOT":
                if top_obi > 0.45:
                    remaining_sell = remaining_sell * 50 // 100
                elif top_obi < -0.45:
                    remaining_buy = remaining_buy * 70 // 100
        ...
        if params.get("triple_edge", False) and remaining_buy >= 3 and remaining_sell >= 3:
            # v39 Codex : ladder paramétrée — triple_edge classique OU far_weights/offsets si one-sided
            pyramid_pct = params.get("pyramid_pct", (55, 30))
            p1, p2 = pyramid_pct
            normal_weights = (p1, p2, max(0, 100 - p1 - p2))
            normal_offsets = (0, 1, 2)
            bid_weights = params.get("far_weights", normal_weights) if far_bid_mode else normal_weights
            ask_weights = params.get("far_weights", normal_weights) if far_ask_mode else normal_weights
            bid_offsets = params.get("far_offsets", normal_offsets) if far_bid_mode else normal_offsets
            ask_offsets = params.get("far_offsets", normal_offsets) if far_ask_mode else normal_offsets
            _emit_ladder(orders, product, "BUY", make_bid_price, remaining_buy, bid_offsets, bid_weights)
            _emit_ladder(orders, product, "SELL", make_ask_price, remaining_sell, ask_offsets, ask_weights)

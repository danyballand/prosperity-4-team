"""
IMC Prosperity 4 Round 1 — v40 (Linear Utility techniques)
Base : v31 champion (12,159 XIREC live)
Ajouts configurables (OFF par défaut, activer selon validation Codex) :
  - mm_filter_k : volume threshold pour MM-filtered mid (Linear Utility)
  - ar1_theta : correction AR(1) sur mm_mid returns (Linear Utility θ=-0.229)
  - post_at_fair : ordre passif supplémentaire à int(fv) exact (chrispyroberts)
  - lu_clear_tier : tier clear Linear Utility (post passive à fv+clear_width)
  - soft_position_skew : skew accru quand |pos| > soft_limit (Linear Utility)
"""
import json
...
DEFAULT_PARAMS = {
    ...
    "min_wall_volume": 10,
    "fixed_fv": None,
    # v40 — Linear Utility / chrispyroberts techniques (OFF by default)
    "mm_filter_k": 0,             # 0 = off, sinon filtre levels avec |vol| >= K
    "mm_use_as_fv": False,        # si True, remplace fv par mm_mid filtré
    "ar1_theta": 0.0,             # 0 = off ; Linear Utility uses -0.229 on Starfruit
    "ar1_blend": 1.0,             # weight applied to AR(1) correction
    "post_at_fair": False,        # post 3rd passive order at int(fv) both sides
    "post_at_fair_qty": 5,
    "lu_clear_tier": False,       # Linear Utility passive clear at fv+clear_width
    "lu_clear_width": 50,
    "lu_soft_position": 40,       # threshold |pos| to activate clear tier
    "soft_position_skew": False,  # extra skew when |pos| > soft_limit
    "soft_skew_multiplier": 2.0,
    # v40b — Deterministic linear FV (Pepper pattern ARIA Uplink)
    # FV(t) = intercept + slope_per_ts * t, intercept fit from warmup observations
    "deterministic_fv": False,
    "det_fv_slope_per_ts": 0.001,     # observed +1000/Mts cross-day & live
    "det_fv_warmup": 50,              # # of mid observations before locking intercept
    "det_fv_residual_clip": 10.0,     # if observed - predicted > clip, trust observation temporarily
    "det_fv_fallback_wall": True,     # during warmup, fall back to wall_mid
}
...
PRODUCT_PARAMS: Dict[str, dict] = {
    ...
    "INTARIAN_PEPPER_ROOT": {
        **DEFAULT_PARAMS,
        ...
        # v40b — Deterministic linear FV (ARIA Uplink pattern, R²=0.99995 3 days CSV, 0.993 live)
        # Slope +1000.00/Mts historique, +1000.097/Mts live ≃ +0.001 par timestamp
        # OFF par défaut ; activé via backtest A/B
        "deterministic_fv": False,
        "det_fv_slope_per_ts": 0.001,
        "det_fv_warmup": 50,
        "det_fv_residual_clip": 10.0,
        "trend_guard": True,
...
def _mm_filtered_mid(depth: OrderDepth, min_vol: int) -> Optional[float]:
    """Linear Utility : garde uniquement les levels avec |volume| >= min_vol,
    puis retourne (min_filtered_ask + max_filtered_bid) / 2. None si pas assez."""
    if min_vol <= 0:
        return None
    filt_asks = [p for p, q in depth.sell_orders.items() if abs(q) >= min_vol]
    filt_bids = [p for p, q in depth.buy_orders.items() if abs(q) >= min_vol]
    if not filt_asks or not filt_bids:
        return None
    return (min(filt_asks) + max(filt_bids)) / 2.0


def _lu_ar1_correction(product: str, mm_mid: float, params: dict, trader_data: dict) -> float:
    """Linear Utility AR(1) mean-reversion correction :
       fair = mm_mid + mm_mid * last_return * theta
    Grid-searched θ=-0.229 sur Starfruit 2024. À re-calibrer sur Pepper."""
    theta = float(params.get("ar1_theta", 0.0))
    if theta == 0.0:
        return 0.0
    blend = float(params.get("ar1_blend", 1.0))
    pstate = trader_data.setdefault(product, {})
    last_mm = pstate.get("last_mm_mid")
    pstate["last_mm_mid"] = mm_mid
    if last_mm is None or last_mm <= 0:
        return 0.0
    last_ret = (mm_mid - last_mm) / last_mm
    return blend * mm_mid * last_ret * theta
...
def _deterministic_fv(product: str, depth: OrderDepth, params: dict, trader_data: dict, timestamp: int) -> Optional[float]:
    """
    v40b — FV déterministe linéaire (pattern ARIA Uplink).
    Observation: Pepper mid(t) = intercept_day + slope*t avec R²=0.99995 sur 3 jours CSV
    et R²=0.993 sur le live, slope ≃ +0.001 par timestamp (+1000/Mts).

    Stratégie:
      1. Warmup N obs (default 50) : accumule wall_mids, fallback = wall_mid
      2. Après warmup : lock intercept = mean(mid - slope*ts) sur la fenêtre
      3. Retourne intercept + slope * timestamp (FV déterministe)

    Le slope est connu a priori (0.001) — on ne le fit pas pour éviter overfit
    sur les 50 premiers samples (variance trop grande sur petite fenêtre).
    """
    slope = float(params.get("det_fv_slope_per_ts", 0.001))
    warmup = int(params.get("det_fv_warmup", 50))
    fallback_wall = bool(params.get("det_fv_fallback_wall", True))

    pstate = trader_data.setdefault(product, {})
    det_state = pstate.setdefault("det_fv", {"samples": [], "intercept": None})

    # Always compute current wall_mid (for sampling and fallback)
    wmid = _wall_mid(depth, int(params.get("min_wall_volume", 5)))
    if wmid is None:
        return None

    # If intercept already locked, just predict
    if det_state.get("intercept") is not None:
        intercept = float(det_state["intercept"])
        return intercept + slope * float(timestamp)

    # Still in warmup : accumulate samples
    samples = det_state.setdefault("samples", [])
    samples.append((int(timestamp), float(wmid)))

    if len(samples) >= warmup:
        # Lock intercept : mean of (mid - slope*ts)
        n = len(samples)
        intercept = sum(mid - slope * ts for ts, mid in samples) / n
        det_state["intercept"] = float(intercept)
        # Free memory
        det_state["samples"] = []
        return intercept + slope * float(timestamp)

    # During warmup : fallback to wall_mid (safe, same as base behavior)
    return float(wmid) if fallback_wall else None
...
def trade_product(product: str, state: TradingState, trader_data: dict) -> List[Order]:
    ...
    if params.get("fixed_fv") is not None:
        ...
        if params.get("adaptive_fixed_fv", False):
            ...
            if wmid is not None:
                ...
                fv += blend * delta
    elif params.get("deterministic_fv", False):
        fv = _deterministic_fv(product, depth, params, trader_data, state.timestamp)
    ...
    # v40 — Linear Utility MM-filtered mid + AR(1) correction
    mm_k = int(params.get("mm_filter_k", 0))
    if mm_k > 0:
        mm_mid = _mm_filtered_mid(depth, mm_k)
        if mm_mid is not None:
            # Optionally replace fv with mm_mid (only if explicitly allowed)
            if params.get("mm_use_as_fv", False):
                fv = mm_mid
            fv = float(fv) + _lu_ar1_correction(product, mm_mid, params, trader_data)
    ...
    # v40 — Linear Utility soft position skew : extra push quand |pos| > soft
    if params.get("soft_position_skew", False):
        soft = int(params.get("lu_soft_position", 40))
        if abs(position) > soft:
            excess = abs(position) - soft
            mult = float(params.get("soft_skew_multiplier", 2.0))
            skew += -float(position - target_bias) / max(1, abs(position)) * excess * skew_coef * (mult - 1.0)
    ...
    # v40 — Linear Utility CLEAR tier : post passive à fv ± clear_width quand |pos| > soft
    # Contrairement à inventory_clearing (qui TAKE), ce tier POST un make order passif
    if params.get("lu_clear_tier", False) and not near_limit:
        cur_pos = position + buy_used - sell_used
        soft = int(params.get("lu_soft_position", 40))
        clear_w = int(params.get("lu_clear_width", 50))
        fv_i = int(round(fv))
        if cur_pos > soft:
            # long → post sell passif à fv + clear_width pour cycler
            px = fv_i + clear_w
            qty = min(cur_pos - soft, max(0, limit + position - sell_used))
            if qty > 0:
                orders.append(Order(product, px, -qty))
                sell_used += qty
        elif cur_pos < -soft:
            px = fv_i - clear_w
            qty = min(-cur_pos - soft, max(0, limit - position - buy_used))
            if qty > 0:
                orders.append(Order(product, px, qty))
                buy_used += qty

    # v40 — POST AT FAIR (chrispyroberts) : ordre passif à int(fv) exact des deux côtés
    if params.get("post_at_fair", False) and not near_limit:
        paf_qty = int(params.get("post_at_fair_qty", 5))
        fv_i = int(round(fv))
        rem_b = max(0, limit - (position + buy_used))
        rem_s = max(0, limit + (position - sell_used))
        qb = min(paf_qty, rem_b)
        qs = min(paf_qty, rem_s)
        if qb > 0:
            orders.append(Order(product, fv_i, qb))
            buy_used += qb
        if qs > 0:
            orders.append(Order(product, fv_i, -qs))
            sell_used += qs

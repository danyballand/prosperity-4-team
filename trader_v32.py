"""
IMC Prosperity 4 Round 1 — v32
Base : v31 champion (MD5 a45f0d686e53172163e08ef9dad0081c, +12,159 live R1)
Ajouts v32 :
  - OFI (Cont-Kukanov-Stoikov 2014) → micro-correction FV Osm. α=2.0 hl=50.
    Backtest 3j : +118 XIREC vs v31 baseline, sign-flip validé (réel, pas noise).
  - Toxicity tracker (markout EWMA sur own_trades) → widen make_edge quand
    picked-off. INACTIVABLE en backtest (own_trades vide) mais neutre par défaut.
  - Queue-skip (pro-rata adapted) : désactivé par défaut, testé -185 XIREC.
"""
import json
...
PRODUCT_PARAMS: Dict[str, dict] = {
    "ASH_COATED_OSMIUM": {
        **DEFAULT_PARAMS,
        ...
        # v32 : OFI Osm (Cont-Kukanov-Stoikov 2014). Backtest 3j +118 XIREC vs v31.
        "use_ofi_correction": True,
        "ofi_halflife": 50.0,
        "ofi_alpha": 2.0,
        # v32 : toxicity tracker LIVE-ONLY (own_trades vide en backtest, harmless).
        "use_toxicity": True,
        "tox_halflife": 30.0,
        "tox_horizon": 20,
        "tox_theta": 1.0,
        "tox_cap": 1.5,
        "use_queue_skip": False,  # testé -185, inactif
        "queue_skip_threshold": 0,
    },
...
# ==============================================================================
# v32 ADD-ONS : OFI, Toxicity, Queue-skip (pro-rata adapted)
# ==============================================================================

def _update_ofi(product: str, depth: OrderDepth, trader_data: dict, params: dict) -> float:
    """
    Order Flow Imbalance (Cont, Kukanov, Stoikov 2014).
    OFI_t = delta_bid_qty + delta_ask_qty selon les mouvements de prix best.
    Formule exacte par composante :
        e_n =  q_b,t          si p_b,t > p_b,t-1
        e_n =  q_b,t - q_b,t-1 si p_b,t == p_b,t-1
        e_n = -q_b,t-1        si p_b,t < p_b,t-1
    Symétrique du côté ask (signe opposé : ask qty monte → pression vendeuse).
    EWMA pour smoother. Retourne le signal normalisé dans [-1, +1] approx.
    """
    pstate = trader_data.setdefault(product, {})
    ofi_st = pstate.setdefault("_ofi", {"pb": None, "qb": 0, "pa": None, "qa": 0, "ewma": 0.0, "scale": 1.0})
    ...
    qb = int(depth.buy_orders.get(bb, 0))
    qa = int(abs(depth.sell_orders.get(ba, 0)))

    e_b = 0.0
    if ofi_st["pb"] is not None:
        if bb > ofi_st["pb"]:
            e_b = float(qb)
        elif bb == ofi_st["pb"]:
            e_b = float(qb - ofi_st["qb"])
        else:
            e_b = -float(ofi_st["qb"])
    e_a = 0.0
    if ofi_st["pa"] is not None:
        if ba < ofi_st["pa"]:
            e_a = float(qa)
        elif ba == ofi_st["pa"]:
            e_a = float(qa - ofi_st["qa"])
        else:
            e_a = -float(ofi_st["qa"])

    ofi_flow = e_b - e_a

    hl = max(1.0, float(params.get("ofi_halflife", 10.0)))
    alpha = 1.0 - 0.5 ** (1.0 / hl)
    ofi_st["ewma"] = (1.0 - alpha) * ofi_st["ewma"] + alpha * ofi_flow

    # adaptive normalization : running magnitude so the scale stays ~ order of 1
    mag = abs(ofi_st["ewma"])
    ofi_st["scale"] = max(1.0, 0.99 * ofi_st["scale"] + 0.01 * max(mag, 1.0) * 5.0)

    ofi_st["pb"], ofi_st["qb"], ofi_st["pa"], ofi_st["qa"] = bb, qb, ba, qa

    s = max(1e-6, ofi_st["scale"])
    normalized = max(-1.5, min(1.5, ofi_st["ewma"] / s))
    return normalized


def _update_toxicity(product: str, state: TradingState, trader_data: dict,
                     params: dict, fv: float) -> float:
    """
    Toxic flow tracker. Pour chaque own_trade on calcule un markout à tox_horizon ticks :
        markout = sign(qty) * (fv_future - price_fill)
    Si la moyenne EWMA des markouts devient négative de > tox_theta ticks,
    on est adversely selected. Retourne un multiplier ∈ [1, tox_cap] à appliquer à make_edge.
    """
    pstate = trader_data.setdefault(product, {})
    tox = pstate.setdefault("_tox", {"pending": [], "ewma": 0.0})

    horizon = int(params.get("tox_horizon", 20))
    hl = max(1.0, float(params.get("tox_halflife", 30.0)))
    alpha = 1.0 - 0.5 ** (1.0 / hl)
    theta = float(params.get("tox_theta", 1.0))
    cap = float(params.get("tox_cap", 1.5))

    # 1) collect our fresh own_trades into pending
    own = state.own_trades.get(product, []) or []
    for tr in own:
        # own_trades of current tick correspond to fills from last tick; process once
        tts = getattr(tr, "timestamp", state.timestamp)
        key = f"{tts}|{tr.price}|{tr.quantity}"
        seen = tox.setdefault("_seen", [])
        if key in seen:
            continue
        seen.append(key)
        if len(seen) > 200:
            del seen[: len(seen) - 200]
        # qty sign: buys by us → positive (bought at price, want fv_future > price)
        # tr.quantity as reported in own_trades is signed from our perspective on some envs,
        # otherwise we infer from buyer field. Use sign-safe form using state.position delta is hard here ;
        # we rely on sign of qty (standard IMC convention : own_trades qty is absolute, use buyer==SELF)
        buyer = getattr(tr, "buyer", "") or ""
        seller = getattr(tr, "seller", "") or ""
        q = abs(int(tr.quantity))
        if buyer == "SUBMISSION" or (buyer and not seller):
            side = 1  # we bought
        elif seller == "SUBMISSION" or (seller and not buyer):
            side = -1  # we sold
        else:
            # Fall-back : use sign of quantity if negative → we sold
            side = 1 if int(tr.quantity) > 0 else -1
        tox["pending"].append({"ts": int(tts), "px": float(tr.price), "side": int(side), "q": q})

    # 2) mature any pending older than horizon → update EWMA
    ts = int(state.timestamp)
    ripe = []
    not_yet = []
    for p in tox["pending"]:
        if ts - p["ts"] >= horizon:
            ripe.append(p)
        else:
            not_yet.append(p)
    for p in ripe:
        markout = p["side"] * (fv - p["px"])
        tox["ewma"] = (1.0 - alpha) * tox["ewma"] + alpha * markout
    tox["pending"] = not_yet

    # 3) multiplier : 1.0 si non-toxique, jusqu'à cap quand markout << -theta
    if tox["ewma"] >= -theta:
        return 1.0
    # linear ramp between -theta and -3*theta
    deficit = min(3.0 * theta, -tox["ewma"]) - theta  # in (0, 2*theta]
    span = max(1e-6, 2.0 * theta)
    frac = deficit / span
    return 1.0 + (cap - 1.0) * frac


def _should_skip_penny(depth: OrderDepth, penny_price: int, side: str,
                       threshold: int) -> bool:
    """
    Queue-jump adverse selection guard.
    Quand le volume au best du côté concerné est épais, pennyiser (bid+1 / ask-1)
    nous isole à un prix meilleur alors qu'un adverse informé peut swipe à travers.
    Résultat : on paye le picked-off pour gagner 1 tick.
    Skip le penny si vol_best > threshold OU si penny_price égale déjà le best
    ...
    if side == "BID":
        ...
        if penny_price <= bb:
            return False  # pas un vrai penny
        vol_best = int(depth.buy_orders[bb])
    else:
        ...
        if penny_price >= ba:
            return False
        vol_best = int(abs(depth.sell_orders[ba]))
    return vol_best > threshold
...
def trade_product(product: str, state: TradingState, trader_data: dict) -> List[Order]:
    ...
    # v32 ADD-ON #1 : OFI correction (Cont-Kukanov-Stoikov 2014)
    # On met toujours à jour l'état OFI (pour qu'il soit prêt dès qu'on l'active),
    # la correction est appliquée uniquement si use_ofi_correction = True
    ofi_norm = _update_ofi(product, depth, trader_data, params)
    if params.get("use_ofi_correction", False):
        fv += float(params.get("ofi_alpha", 0.0)) * ofi_norm

    # v32 ADD-ON #2 : Toxic flow tracker → multiplier on make_edge (utilisé plus bas)
    tox_multiplier = 1.0
    if params.get("use_toxicity", False):
        tox_multiplier = _update_toxicity(product, state, trader_data, params, fv)
    ...
    # MAKE
    if not near_limit:
        make_edge_base = int(params.get("make_edge", 1))
        # v32 ADD-ON #2 : widen edge when own-flow is adversely selected
        if params.get("use_toxicity", False) and tox_multiplier > 1.0:
            make_edge = int(round(make_edge_base * tox_multiplier))
        else:
            make_edge = make_edge_base
        ...
        if params.get("pennying", False):
            ...
            fv_i = int(round(fv))
            # v32 ADD-ON #3 : queue-dilution skip (pro-rata adapted)
            use_queue_skip = bool(params.get("use_queue_skip", False))
            q_thresh = int(params.get("queue_skip_threshold", 0))
            if bb is not None:
                penny_bid = min(bb + 1, fv_i - min_edge)
                skip_bid = use_queue_skip and _should_skip_penny(depth, penny_bid, "BID", q_thresh)
                if not skip_bid:
                    make_bid_price = max(make_bid_price, penny_bid)
            if ba is not None:
                penny_ask = max(ba - 1, fv_i + min_edge)
                skip_ask = use_queue_skip and _should_skip_penny(depth, penny_ask, "ASK", q_thresh)
                if not skip_ask:
